...
export const ServiceList = (props) => (
  <List {...props}>
    <Datagrid rowClick="edit">
      ...
      {/* ✅ Исправленное отображение центров */}
      <FunctionField
        label="Центры"
        render={(record) =>
          record.centers && record.centers.length > 0
            ? record.centers.map((center) => center.title).join(', ')
            : '-'
        }
      />
      ...
      {/* ✅ Отображение первого изображения, если оно есть */}
      <FunctionField
        label="Изображение"
        render={(record) =>
          record.img && record.img.length > 0 ? record.img[0] : 'Нет изображения'
        }
      />
