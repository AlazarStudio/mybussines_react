...
import { news } from '../../../../bd'; // Ваш массив новостей
import { Link, useLocation, useParams } from 'react-router-dom';

function OneNewsPage({ children, ...props }) {
  const location = useLocation();
  const { newsId } = useParams(); // Получаем ID или заголовок новости из параметров маршрута
  ...
  // Ищем новость по ID или заголовку
  const currentNews = news.find(
    (item) =>
      item.id === newsId || item.title.replaceAll(' ', '-').toLowerCase() === newsId
  );
      ...
      <CenterBlock>
        <WidthBlock>
          <div className={classes.container}>
            ...
            {/* Отображение новости */}
            {currentNews ? (
              <div className={classes.containerNews} key={currentNews.id}>
