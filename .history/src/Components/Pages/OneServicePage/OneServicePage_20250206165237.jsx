...
function OneServicePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setService(foundService || null); // Если не найдено, устанавливаем null
      } catch (err) {
  ...
  const currentService = service.find(
    (item) =>
      ...
      <WidthBlock>
        <div className={classes.container}>
          ...
          <div className={classes.containerNews}>
            ...
            <img
              src={`${uploadsConfig}${currentService.img[0]}`}
