...
export default function SupportPageAll({ activeTab }) {
  ...
  const filteredSupportsType = supports.filter((support) => {
    if (activeTab === 'federal') {
      return support.typeSupport.title === 'Федеральная мера поддержки'; // 1 - ID для "Федеральные"
    }
    if (activeTab === 'regional') {
      return support.typeSupport.title === 'Региональная мера поддержки'; // 2 - ID для "Региональные"
    }
    return false;
  });
