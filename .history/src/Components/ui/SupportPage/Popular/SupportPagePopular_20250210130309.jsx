...
function SupportPagePopular({ activeTab }) {
  ...
  const filteredSupports = supports.filter((support) => {
    if (activeTab === 'federal') {
      return support.typeSupport.title === 'Федеральная мера поддержки' && support.popular === true; // 1 - ID для "Федеральные"
    }
    if (activeTab === 'regional') {
      return support.typeSupport.title === 2 && support.popular === true; // 2 - ID для "Региональные"
    }
    return false;
  });
