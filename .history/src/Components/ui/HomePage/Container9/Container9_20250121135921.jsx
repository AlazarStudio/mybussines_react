...
function Container9({ children, ...props }) {
  ...
  const totalUl = Object.values(regionInfo).reduce((sum, region) => {
    return sum + Number(region.ul); // Преобразуем значение ip в число и суммируем
  }, 0);

  const totalSmsp = Object.values(regionInfo).reduce((sum, region) => {
    return sum + Number(region.sm); // Преобразуем значение ip в число и суммируем
  }, 0);
