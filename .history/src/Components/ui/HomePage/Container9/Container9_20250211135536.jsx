...
function Container9() {
  ...
  const regionsWithoutCherkessk = regionsWithoutSamozan.filter((r) => r.title !== 'г.Черкесск');
  const samozanRegion = regionInfo.find((r) => r.title === 'Самозанятые');

  // ✅ Подсчет значений
  const totalIp = regionsWithoutSamozan.reduce((sum, region) => sum + Number(region.ip || 0), 627);
  ...
  const totalIpReg = regionsWithoutCherkessk.reduce((sum, region) => sum + Number(region.ip || 0), 627);
