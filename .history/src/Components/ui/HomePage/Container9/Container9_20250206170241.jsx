...
function Container9({ children, ...props }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${serverConfig}/api/maps`);
  ...
  // ✅ Фильтрация данных (Самозанятые отдельно)
  ...
  // ✅ Подсчет значений
  const totalIp = regionsWithoutSamozan.reduce((sum, region) => sum + Number(region.ip), 0);
  const totalUl = regionsWithoutSamozan.reduce((sum, region) => sum + Number(region.ul), 0);
  const totalSmsp = regionsWithoutSamozan.reduce((sum, region) => sum + Number(region.smsp), 0);

  const totalSmspSamozan = samozanRegion?.smsp || 0;
